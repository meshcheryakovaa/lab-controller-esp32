...
### Выгрузка сегментов лога (M15)

```text
POST /api/v1/logs/start          + storage_mode, segment_bytes, collector_id
GET  /api/v1/logs/{id}/segments                     что ждёт передачи
GET  /api/v1/logs/{id}/segments/{n}/export.csv      потоковая отдача файла
POST /api/v1/logs/{id}/segments/{n}/ack             единственное, что удаляет
```

`storage_mode` отсутствует — значит `single`, то есть прежнее поведение.

`GET .../segments` — **источник истины**. WebSocket-событие `LOG_SEGMENT_READY`
только ускоряет передачу; клиент, пропустивший его, восстанавливает весь список
из этого ответа, поэтому потерянный кадр не может стать потерянным сегментом.

`GET .../segments/{n}/export.csv` отдаётся прямо из файла и несёт
`Content-Length`, `ETag: "crc32-…"` и `X-Payload-CRC32`. Сегмент в состоянии
`WRITING` не отдаётся: у него ещё нет футера, а значит и итоговой суммы.

`POST .../ack` требует тех же прав, что удаление набора, и проверяет владельца,
состояние `READY`, размер и контрольную сумму. Повторный ACK возвращает
`already_acknowledged` вместо ошибки: ответ на первый мог потеряться.

### Домашняя сеть (M16)

Всё под `/api/v1/network`. Правило раздела — ADR-0022: изменение сетевых
настроек не должно делать прибор недоступным.
...
**Пароля здесь нет и быть не может.** В структуре `NetworkStatus` отсутствует
поле для него, поэтому сериализатор не способен его выдать — ни здесь, ни в
`/diagnostics`, ни в экспорте конфигурации. `password_set` отвечает на
единственный нужный вопрос, не перенося секрет.

`mdns` даётся **вместе** с `station.ip`, а не вместо: `.local` разрешается не на
каждой машине, и прибор, сообщающий только имя, недостижим на практике.

| Состояние | Что означает |
|---|---|
| `AP_ONLY` | своя точка доступа, домашняя сеть не настроена |
| `STA_CONNECTING` | идёт подключение или проверка реквизитов |
| `STA_CONNECTED` | адрес получен, прибор в домашней сети |
| `AP_STA_FALLBACK` | сеть потеряна: повторы идут, дверь открыта |
| `NETWORK_ERROR` | не поднялось ничего; датчики продолжают работать |
...
```json
{"accepted": true, "state": "STA_CONNECTING"}
```

Ответ — **`202 Accepted`, не `200`**: принято, а не выполнено. Подключение
занимает до 15 секунд, и обработчик, который бы их ждал, остановил бы тот самый
опрос `GET /api/v1/network`, которым страница следит за результатом — они
приходят на одну и ту же задачу HTTP.

Реквизиты записываются как pending и заменяют рабочие только после того, как
роутер выдал адрес. Неверный пароль оставляет прежнюю сеть настроенной, а
резервную AP — включённой.

| Ситуация | Ответ |
|---|---|
| пустой SSID | `400 INVALID_ARGUMENT`, `error.field = "ssid"` |
| SSID длиннее 32 байт | `413 PAYLOAD_TOO_LARGE` |
| пароль короче 8 символов | `400`, `error.field = "password"` |
| пароль пустой | принимается — открытая сеть |
| подключение уже идёт | `409 RESOURCE_BUSY` |
| нет прав записи | `401 UNAUTHORIZED` |

#### Сканирование

```http
POST /api/v1/network/scan     → {"state": "SCANNING"}
GET  /api/v1/network/scan     → {"state": "COMPLETE", "networks": [...]}
```

Асинхронное. Повторный `POST` во время сканирования не запускает второе, а
возвращает текущее состояние. Не более 20 сетей, по убыванию RSSI, одинаковые
SSID объединяются с сохранением сильнейшего сигнала. Сканирование во время
проверки реквизитов — `409`: радио занято тем, чего ждёт оператор.

#### Забыть сеть и переименование

```http
DELETE /api/v1/network/config
```

```json
{"cleared": true, "state": "AP_ONLY", "ip": "192.168.4.1",
 "ssid": "LAB-CONTROLLER-A1B2C3"}
```

В ответе есть адрес, **куда переходить**. Сообщить, что старый адрес пропал, не
сказав нового, — то же самое, что сделать прибор недоступным.

```http
PUT /api/v1/network/hostname
{"hostname": "lab-reactor"}
```

От 1 до 31 символа `a-z`, `0-9` и дефис, не начинается и не заканчивается
дефисом, без точек. Проверяется предикатом `INetworkManager::hostnameIsValid()`,
общим для API и прошивки: две копии правила — это два правила, и обновляют
всегда не то.
